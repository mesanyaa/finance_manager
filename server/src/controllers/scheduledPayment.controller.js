...
export const getScheduledPayments = async (req, res) => {
  try {
    const payments = await ScheduledPayment.find({ user: req.user._id })
      .sort({ date: 1 });
    res.json(payments);
  } catch (error) {
    res.status(500).json({ message: 'Error fetching scheduled payments', error: error.message });
  }
};
...
export const createScheduledPayment = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error creating scheduled payment', error: error.message });
  }
};
...
export const deleteScheduledPayment = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error deleting scheduled payment', error: error.message });
  }
}; 